    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        if (_searchResult[index].CASE_STATUS != null) {
          if (_searchResult[index].CASE_STATUS.trim().endsWith("รอรับคดี")) {
          ...
          } else if (_searchResult[index].CASE_STATUS.trim().endsWith(
              "ไม่รับเป็นคดี")) {
            track_type = 1;
            title_code = "เลขใบงานจับกุม";
            title_person = "ผู้กล่าวหา";
            ...
            String name = "";
            _searchResult[index].LawbreakerList.forEach((item) {
              name = item.ARREST_LAWBREAKER_NAME.toString() + "\n";
            });
            data_person = name;
          } else if (_searchResult[index].CASE_STATUS.trim().endsWith(
              "รอพิสูจน์ของกลาง")) {
                    ...
                    Column(
                      ...
                      children: <Widget>[
                        Container(
                          ...
                          child: Text(
                            title_code, style: textLabelStyle,),
                        ),
                        Padding(
                        ...
                        Container(
                          padding: paddingLabel,
                          child: Text(
                            title_person, style: textLabelStyle,),
                        ),
                        Padding(
                          padding: paddingInputBox,
                          child: Text(
    ...
    await new TrackingFuture().apiCaseStatusgetByCon(map).then((onValue) {
      ...
      if (onValue != null && onValue.CASE_STATUS.length > 0) {
        ...
        if (_itemsTrackingList.ARREST_ID != null) {
          ...
          itemsTimeLineList.add(new ItemsTimeLineList(
              "เลขที่ใบงานจับกุม",
              _itemsTrackingList.ARREST_CODE,
              "วันที่เกิดเหตุ",
              _convertDate(_itemsTrackingList.OCCURRENCE_DATE)+" "+_convertTime(_itemsTrackingList.OCCURRENCE_DATE),
              "ผู้ต้องหา",
              law_name
          ));
        }
        ...
        if (_itemsTrackingList.LAWSUIT_NO != null) {
          ...
          itemsTimeLineList.add(new ItemsTimeLineList(
              "เลขรับคำกล่าวโทษ",
              _itemsTrackingList.LAWSUIT_NO,
              "วันที่รับคำกล่าวโทษ",
              _itemsTrackingList.LAWSUIT_DATE!=null
                  ?(_convertDate(_itemsTrackingList.LAWSUIT_DATE)+" "+_convertTime(_itemsTrackingList.LAWSUIT_DATE))
                  :"",
              "ผู้ต้องหา",
              law_name));
        }
        if (_itemsTrackingList.PROVE_NO != null) {
          itemsTimeLineList.add(new ItemsTimeLineList(
              "ทะเบียนตรวจพิสูจน์ ",
              _itemsTrackingList.PROVE_NO,
              "วันที่พิสูจน์ของกลาง",
              _convertDate(_itemsTrackingList.PROVE_DATE)+" "+_convertTime(_itemsTrackingList.PROVE_DATE),
              "ผู้พิสูจน์ของกลาง",
              _itemsTrackingList.PROVE_STAFF_NAME));
        }
        if (_itemsTrackingList.COMPARE_NO != null) {
          itemsTimeLineList.add(new ItemsTimeLineList(
              "เลขเปรียบเทียบคดี ",
              _itemsTrackingList.COMPARE_NO,
              "วันที่พิสูจน์ของกลาง",
              _convertDate(_itemsTrackingList.COMPARE_DATE)+" "+_convertTime(_itemsTrackingList.COMPARE_DATE),
              "ผู้เปรียบเทียบดคี",
              _itemsTrackingList.COMPARE_STAFF_NAME));
        }
      }
    });
                    ...
                    child: _searchResult.length>0
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          Container(
                            padding: paddingLabel,
                            child: Text('รวมทั้งหมด '+formatter.format(_searchResult.length).toString()+' คดี',style: textLabelStyle,),
                          )
                        ],
                      ),
                    )
